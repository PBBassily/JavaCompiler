...
struct thread
  {
    ...
    struct semaphore waitSema; // used in wait function to wait if one of the children`s thread running
    struct semaphore execSema; // used in execure the parent process should wait for its children to be executed

    struct file* held_file;
...
/* this struct hold the information about the child thread that the parent kept them in children list*/
struct child
{
  ...
  bool somethread_is_waiting_on_me; // boolean to know if the parent wait twice on the same child (ERROR)
  bool loaded; // boolean indicate that the child thread successfully loaded
};

/* this struct hold the information about the file that the parent kept hash map of it*/
struct fd_mapper
{
  int fd; // id for the file
  struct file* file; // struct file
  struct hash_elem hash_elem; // hash elemenet used in hash map
};
